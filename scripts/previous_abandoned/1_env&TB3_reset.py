...
def reset_env(env_id, scene_type, env_origin, handles, rng, robots=None, base_view=None):
    """Reset environment and TB3 robot"""
    ...
    if robots is not None and base_view is not None:
        # Determine spawn position based on scene type
        if scene_type == SCENE_EMPTY or scene_type == SCENE_CYLINDER:
            ...
            # Box and Door: spawn at local (-2.5, -2.5), facing (0, 0)
            local_x = 2.5
            local_y = 2.5
            # Calculate yaw to face (0, 0)
            dx = 0.0 - local_x
            dy = 0.0 - local_y
        ...
        spawn_pos = np.array([
            ...
            0.0350  # Slight elevation to avoid ground collision
        ], dtype=np.float32)
        
        spawn_rot = quat_wxyz_from_yaw(np.array([spawn_yaw], dtype=np.float32))
        
        # Set robot pose (single robot)
...
def main():
    ...
    # Create RigidPrimView for base_link (for mass and COM adjustment)
    ...
    if robots.count != num_envs:
        print(f"[WARN] robots.count={robots.count}, expected={num_envs}. Check prim_paths_expr.")

    # Configure wheel joints (from baseline)
    ...
    if left_idx is None or right_idx is None:
        print("[WARN] Could not find wheel joints. Continuing without wheel configuration.")
    else:
        wheel_kp = 0.0
        wheel_kd = 260.0
    ...
    # Configure base mass and COM (from baseline)
    base_mass = 2.2
    com_z = -0.10
    com_x = -0.08
    ...
    for i in range(num_envs):
        reset_env(i, scene_types[i], env_origins[i], handles[i], rng, robots, base_view)
    ...
    print("[INFO] TB3 spawn: empty/cylinder at (0,0) random yaw, box/door at (-2.5,-2.5) facing (0,0)")
