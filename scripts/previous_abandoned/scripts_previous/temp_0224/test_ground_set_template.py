...
def main():
    ...
    class SimpleWaffleTask(RLTask):
        ...
        def pre_physics_step(self, actions):
            ...
            # ⚠️ 关键修复：参考 OIGE 官方示例 ingenuity.py（第169-171行）
            # 官方示例的模式：在 post_reset 中初始化 dof_vel，在 pre_physics_step 中修改并设置
            # 这确保速度控制命令能正确应用
            ...
            # ⚠️ 关键修复：参考 ingenuity.py 第169-171行的实现方式
            # 使用已初始化的 dof_vel（在 post_reset 中获取），只修改需要的关节
            # 这确保速度控制命令能正确应用
            if hasattr(self, 'dof_vel'):
                ...
                self.dof_vel[:, left_idx] = left_wheel_vel
                self.dof_vel[:, right_idx] = right_wheel_vel
            else:
                # 如果 dof_vel 未初始化，创建新的（向后兼容）
            ...
            if self._vel_set_count < 3:
                ...
                print(f"   dof_vel[0, :] = {self.dof_vel[0, :].cpu().numpy()}")
                self._vel_set_count += 1
            
            # ⚠️ 关键修复：参考 ingenuity.py 第171行的实现方式
            # 直接调用 set_joint_velocities，传入完整的 dof_vel
            # 官方示例证明这种方式在速度控制模式下是有效的
            self.robots.set_joint_velocities(self.dof_vel)
        ...
        def post_reset(self):
            ...
            # ⚠️ 关键修复：参考 OIGE 官方示例 ingenuity.py（第180-181行）
            # 在 post_reset 中初始化 dof_pos 和 dof_vel，这是官方推荐的方式
            # 这确保速度控制命令能正确应用
            ...
            if not hasattr(self, '_post_reset_debug_printed'):
                ...
                print(f"   关节位置 shape = {self.dof_pos.shape}")
                print(f"   关节速度 shape = {self.dof_vel.shape}")
                print(f"   关节位置 (env 0) = {self.dof_pos[0, :].cpu().numpy()}")
                print(f"   关节速度 (env 0) = {self.dof_vel[0, :].cpu().numpy()}")
