...
def main():
    ...
    # Critical fix: drive-mode must not allow "one wheel ~0" -> enforce v - |w|*B/2 >= V_DRIVE_MIN
    V_DRIVE_MIN = 0.07
    ...
    STUCK_STEPS = int(1.0 / physics_dt)
    STUCK_CMD_EPS = 1.5
    STUCK_DIST_EPS = 0.005
    no_improve_cnt = np.zeros((num_envs,), dtype=np.int64)
    ...
    while simulation_app.is_running():
        ...
        # Turn state with N-frame confirmation (prevents oscillation + unsafe early drive)
        ...
        # If currently not turning, require ENTER_HOLD frames to enter
        enter_now = (~turn_state) & (enter_cnt >= ENTER_HOLD)
        # If currently turning, require EXIT_HOLD frames to exit
        ...
        # Drive-mode "no-single-wheel-stall" constraint:
        # require v - |w|*B/2 >= V_DRIVE_MIN  => |w| <= 2*(v - V_DRIVE_MIN)/B
        ...
        # If drive_mask but v is too small to satisfy V_DRIVE_MIN, suppress w to 0 (avoid pivot grinding)
        w_cmd = np.where(drive_mask & (v_use < V_DRIVE_MIN), 0.0, w_cmd).astype(np.float32)

        # Build joint velocity targets via official controller
        ...
        # "Too slow == stuck" based on distance improvement under command
        cmd_mag = np.maximum(np.abs(targets[:, left_idx]), np.abs(targets[:, right_idx])).astype(np.float32)
        improving = (dist - dist2) > STUCK_DIST_EPS

        stuck_now = (cmd_mag > STUCK_CMD_EPS) & (~improving) & (~cooldown_mask) & (~reached) & (~collision) & (~timeout)
        no_improve_cnt = np.where(stuck_now, no_improve_cnt + 1, 0).astype(np.int64)
        stuck_reset = no_improve_cnt >= STUCK_STEPS
        ...
        if now - last_print > 0.35:
            ...
            print(
                ...
                f"no_imp_cnt={int(no_improve_cnt[0])} "
