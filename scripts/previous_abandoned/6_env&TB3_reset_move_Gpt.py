...
def main():
    ...
    # Controller (robust, eliminates "low-v high-w single-wheel reverse" zone)
    v_max = 0.26
    w_max_drive = 1.8
    w_max_turn = 1.2
    k_yaw = 3.5
    ...
    SETTLE_STEPS = 8  # apply extra zero-target steps after reset to flush old solver/contact state

    # Turn mode hysteresis (more aggressive: avoids big heading error in drive)
    ENTER_TURN = 55.0 * np.pi / 180.0
    EXIT_TURN = 25.0 * np.pi / 180.0
    turn_state = np.zeros((num_envs,), dtype=bool)

    # Stuck detection (per-env)
    STUCK_STEPS = int(1.2 / physics_dt)   # must be stuck for ~1.2s
    STUCK_VEL_EPS = 0.02                  # base linear/ang velocity threshold
    STUCK_CMD_EPS = 1.5                   # wheel cmd must be "non-trivial"
    STUCK_DIST_EPS = 1e-3                 # dist not improving
    ...
    while simulation_app.is_running():
        ...
        # In turn mode: v=0 (prevents "low-v high-w" pivot/lock)
        ...
        # Feasible w limit to avoid wheel saturation
        w_feasible = (max_wheel_vel * WHEEL_RADIUS - np.abs(v_use)) / (WHEEL_BASE * 0.5)
        w_feasible = np.clip(w_feasible, 0.0, w_max_drive).astype(np.float32)
        ...
        # Post-step: evaluate termination
        ...
        # Stuck detection (uses base velocity + command + dist improvement)
        ...
        cmd_mag = np.maximum(np.abs(left_w), np.abs(right_w)).astype(np.float32)

        improving = (last_dist - dist2) > STUCK_DIST_EPS
        last_dist = dist2.copy()

        stuck_now = (
            (cmd_mag > STUCK_CMD_EPS)
        ...
        if np.any(to_reset):
            ...
            # extra settle steps to flush any old contact/solver state
