...
# Isaac Sim 2022.2.1 (Differential Controller)
...
def main():
    ...
    # Differential controller (official)
    ...
    # Wheel drive tuning (softer + more torque, reduces contact lockups)
    wheel_kp = 0.0
    wheel_kd = 120.0
    ...
    for i in range(num_envs):
        ...
        for p, size, center in wall_defs:
            prim, t_op, s_op = _create_cube(
                ...
                visible=show_visual_walls[i], color=(0.6, 0.6, 0.6), collision=True
            )
    ...
    markers = []
    for i in range(num_envs):
        if scene_types[i] in (SCENE_EMPTY, SCENE_CYLINDER):
    ...
    # Stuck detection (per-env)
    STUCK_STEPS = int(1.2 / physics_dt)
    STUCK_VEL_EPS = 0.02
    STUCK_DIST_EPS = 1e-3
    stuck_count = np.zeros((num_envs,), dtype=np.int64)
    ...
    while simulation_app.is_running():
        ...
        # High-level (v,w) with hard gating to avoid pivot zone:
        # - If turn_state True -> v=0 always
        # - Else -> normal drive, and we enforce curvature limit to keep both wheels forward
        ...
        # During cooldown, hard stop all commands
        v_use = np.where(cooldown_mask, 0.0, v_use).astype(np.float32)
        w_cmd = np.where(cooldown_mask, 0.0, w_cmd).astype(np.float32)

        # Curvature limit in drive mode to guarantee both wheels forward (prevents one wheel near 0 / reverse)
        drive_mask = (~turn_state) & (~cooldown_mask)
        w_limit_forward = (2.0 * v_use / WHEEL_BASE) * 0.90
        w_cmd = np.where(drive_mask, np.clip(w_cmd, -w_limit_forward, w_limit_forward), w_cmd).astype(np.float32)

        # DifferentialController -> joint velocities (per env)
        ...
        for i in range(num_envs):
            if cooldown_steps[i] > 0:
                ...
                # DifferentialController forward expects (linear, angular)
                # It returns joint velocities for left/right wheels; we map into our DOFs.
                action = diff_ctrl.forward(command=np.array([float(v_use[i]), float(w_cmd[i])], dtype=np.float32))
                # action is an ArticulationAction; joint_velocities is (2,) for diff drive
                # order in controller is [left, right]
        ...
        # Stuck detection: command present but base not moving and dist not improving
        base_v = robots.get_velocities()
        vx = base_v[:, 0].astype(np.float32)
        vy = base_v[:, 1].astype(np.float32)
        wz = base_v[:, 5].astype(np.float32)
        base_speed = np.sqrt(vx * vx + vy * vy).astype(np.float32)
        base_turn = np.abs(wz).astype(np.float32)

        cmd_mag = np.maximum(np.abs(targets[:, left_idx]), np.abs(targets[:, right_idx])).astype(np.float32)
        improving = (last_dist - dist2) > STUCK_DIST_EPS
        last_dist = dist2.copy()

        stuck_now = (
            (cmd_mag > 1.5)
            & (base_speed < STUCK_VEL_EPS)
            & (base_turn < STUCK_VEL_EPS)
            & (~improving)
            & (~cooldown_mask)
            & (~reached)
            & (~collision)
        ...
        stuck_count = np.where(stuck_now, stuck_count + 1, 0).astype(np.int64)
        stuck_reset = stuck_count >= STUCK_STEPS
        ...
        if np.any(to_reset):
            ...
            # settle steps to flush any residual solver/contact/joint drive state
