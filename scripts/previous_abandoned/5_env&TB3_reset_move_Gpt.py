...
def main():
    ...
    v_max = 0.26
    w_max_drive = 2.2
    w_max_turn = 1.6  # turn-in-place w cap: reduces PhysX lock-ups
    k_yaw = 3.5
    k_v = 0.8
    reach_thresh = 0.18
    ...
    # Turn hysteresis (fixed bug: no blend/turn mismatch)
    ENTER_TURN = 65.0 * np.pi / 180.0
    EXIT_TURN = 35.0 * np.pi / 180.0
    turn_state = np.zeros((num_envs,), dtype=bool)

    # No-reverse constraint only when forward is clearly established
    V_NO_REVERSE_MIN = 0.10
    ...
    while simulation_app.is_running():
        ...
        # Turn hysteresis update (use >= / <= to avoid boundary ambiguity)
        ...
        # No-reverse constraint only when forward is established AND not turning
        forward_stable = (v_use >= V_NO_REVERSE_MIN) & (~turn_state) & (~cooldown_mask)
        ...
        # Scale both together if needed (never per-wheel clip)
