...
def main():
    ...
    # Reset cooldown: after reset, hold commands at 0 for 0.5s
    ...
    while simulation_app.is_running():
        ...
        v_use = (alpha * v_nom * turn_slow).astype(np.float32)

        turn_in_place = abs_yaw > (60.0 * np.pi / 180.0)
        v_use = np.where(turn_in_place, 0.0, v_use).astype(np.float32)

        w_cmd = np.clip(k_yaw * yaw_err, -w_max, w_max).astype(np.float32)

        # hold commands at 0 during cooldown (hard stop)
        v_use = np.where(cooldown_mask, 0.0, v_use).astype(np.float32)
        w_cmd = np.where(cooldown_mask, 0.0, w_cmd).astype(np.float32)

        # avoid single-wheel reverse when in "forward mode" (prevents micro-backup->stuck)
        forward_mask = (v_use > 1e-4) & (~turn_in_place) & (~cooldown_mask)
        w_no_reverse = (2.0 * v_use / WHEEL_BASE) * 0.95
        w_cmd = np.where(forward_mask, np.clip(w_cmd, -w_no_reverse, w_no_reverse), w_cmd).astype(np.float32)
        ...
        if now - last_print > 0.6:
            ...
            print(
                f"E0: dist={dist2[0]:.2f} {reset_reasons[0]} loops={loops[0]} cd={cooldown_steps[0]} | "
                f"E1: dist={dist2[1]:.2f} {reset_reasons[1]} loops={loops[1]} cd={cooldown_steps[1]} | "
                f"E2: dist={dist2[2]:.2f} {reset_reasons[2]} loops={loops[2]} cd={cooldown_steps[2]} | "
                f"E3: dist={dist2[3]:.2f} {reset_reasons[3]} loops={loops[3]} cd={cooldown_steps[3]}"
            )
            last_print = now
