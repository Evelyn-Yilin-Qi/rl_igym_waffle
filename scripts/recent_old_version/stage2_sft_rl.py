"""
Stage2 SFT + RL training script.

- Scene mix: EMPTY/CYLINDER/DOOR/BOX
...
from sim.scenes import SCENE_EMPTY, SCENE_BOX, SCENE_CYLINDER, SCENE_DOOR, yaw_from_quat_wxyz, quat_wxyz_from_yaw, wrap_to_pi
...
def train_supervised_policy(rl_agent, obs_np, act_np, epochs=300, batch_size=2048):
    if len(obs_np) == 0:
...
def main(checkpoint_path=None):
    ...
    checkpoint_dir_stage1 = os.path.join(PROJECT_ROOT, "checkpoints", "ppo_stage1_sft_rl")
    _ = load_checkpoint(rl_agent, checkpoint_path=checkpoint_path, checkpoint_dir=checkpoint_dir_stage1)
    ...
    supervised_obs = []
    supervised_actions = []
    checkpoint_dir = os.path.join(PROJECT_ROOT, "checkpoints", "ppo_stage2_sft_rl")
    writer = SummaryWriter(log_dir=os.path.join(PROJECT_ROOT, "runs", "ppo_stage2_sft_rl"))
    temp_cur_act = np.zeros((num_envs, 2), dtype=np.float32)

    print(f"[Stage2-SFT-RL] start, supervised_steps={SUPERVISED_STEPS}, supervised_epochs={SUPERVISED_EPOCHS}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage2 SFT+RL training")
