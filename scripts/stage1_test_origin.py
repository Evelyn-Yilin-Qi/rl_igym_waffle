...
class Actor(nn.Module):
    ...
    def __init__(self, lidar_dim=36, state_dim=8, act_dim=2, hidden_dim=128):
...
class Critic(nn.Module):
    ...
    def __init__(self, lidar_dim=36, state_dim=8, hidden_dim=128):
...
# ==================== 模型保存工具 ====================
def save_model(ppo, checkpoint_dir, tag):
    """保存Actor/Critic权重到指定目录"""
...
def compute_total_reward(dist, yaw_err, v_cmd, collision, reached, timeout,
    ...
    reward_components = {
        ...
        "static_pen": static_pen,
        "obstacle_pen": obstacle_pen,
        "heading_pen": heading_pen,
...
def main():
    ...
    # ==================== 初始化PPO ====================
    obs_dim = 36 + 2 + 2 + 4  # LiDAR(36) + UserIntent(2) + BaseVel(2) + ActionHistory(4)
    act_dim = 2  # 线速度v、角速度w（归一化到[-1,1]）
    ppo = PPO(
        lidar_dim=36,          # LiDAR分支维度
        state_dim=8,           # 状态特征分支维度（2+2+4）
        act_dim=act_dim,
        lr=3e-4,
        gamma=0.98,
        lamda=0.90,
    ...
    checkpoint_dir = os.path.join(PROJECT_ROOT, "checkpoints", "ppo_stage1_test")
    writer = SummaryWriter(log_dir=os.path.join(PROJECT_ROOT, "runs", "ppo_stage1_test"))
...
torch.tensor(act).unsqueeze(0).to(device)).sum().detach().cpu().numpy()
        ...
        rewards, reward_components = rewards
        # 累计奖励分量用于TB统计
        ...
        if step_count % update_freq == 0 and len(ppo.buffer["obs"]) >= ppo.batch_size:
            ...
            if update_count % save_every == 0:
                save_model(ppo, checkpoint_dir, f"epoch_{update_count:03d}")

            # 达到总轮数，保存最终模型并退出
            if update_count >= total_epochs:
                save_model(ppo, checkpoint_dir, "final")
                break
