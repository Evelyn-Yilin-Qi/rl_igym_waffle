...
def main():
    ...
    # Stuck detection: split into TURN and DRIVE, so we don't reset during legitimate turning
    STUCK_STEPS = int(1.2 / physics_dt)

    # DRIVE stuck (must be moving forward-ish, but dist not decreasing)
    DRIVE_CMD_EPS = 2.0
    DRIVE_V_EPS = 0.03
    DRIVE_DIST_EPS = 0.003

    # TURN stuck (must be commanding turn, but yaw not improving AND base wz ~0)
    TURN_CMD_EPS = 2.0
    TURN_WZ_EPS = 0.03
    TURN_YAW_EPS = 0.01
    ...
    while simulation_app.is_running():
        ...
        # Turn state with N-frame confirmation
        ...
        # DRIVE stuck: not in turn, commanding meaningful wheels, v_use meaningful, but dist not improving
        drive_stuck = (
            (~turn_state)
            & (~cooldown_mask)
        ...
        # TURN stuck: in turn, commanding wheels, but yaw not improving AND base wz ~0 (true lockup)
        turn_stuck = (
            (turn_state)
            & (~cooldown_mask)
