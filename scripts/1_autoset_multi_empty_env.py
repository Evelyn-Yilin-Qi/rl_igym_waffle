"""
Auto-set multi-env loader: TB3 empty scenes only.
num_envs and env_layout are explicit; other spacing defaults remain untouched.
"""
import os
import sys
import numpy as np
...
def validate_layout(num_envs: int, layout):
...
def main():
    ...
    cols, rows = validate_layout(num_envs, env_layout)
    ...
    # Control/readouts from cfg（与 modular 对齐）
    ...
    print(f"[INFO] Loaded {num_envs} empty envs with layout {env_layout} (cols x rows).")
    ...
    while simulation_app.is_running():
        ...
        if now - last_print > 0.35:
            ...
            print(
                f"[E0] scene=EMPTY loop={int(loops[0])} cd={int(cooldown_steps[0])} "
