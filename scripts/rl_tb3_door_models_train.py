"""
固定场景 door：依次用 5 个预训练 ``best_actor_*.pth`` 各跑一遍 ``rl_tb3_door.py``（每轮新进程）。

**不要用 ``isaac_python`` 运行本编排脚本**（见 ``rl_tb3_subprocess_runner`` 说明）。请用::

    python3 scripts/rl_tb3_door_models_train.py
"""
...
def main() -> None:
    ...
    script = os.path.join(PROJECT_ROOT, "scripts", "rl_tb3_door.py")
    for model in MODELS:
