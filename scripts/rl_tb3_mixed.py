"""
SFT + PPO：混合场景（多环境），按 ``empty / cylinder / door / box`` 循环分配，
与 ``stage2_rl_sft.py`` 中 ``scene_list[i % 4]`` 思路一致；单环境观测与 ``rl_tb3_box.py`` 相同。

单跑指定骨干::

    isaac_python scripts/rl_tb3_mixed.py --model cnn_lstm_sft

checkpoint 目录名使用 ``mixed_{model}``。默认 ``num_envs=4``（每种场景 1 个 env），
可在 ``main()`` 顶部改大（建议为 4 的倍数）。
"""
import argparse
import os
import sys
...
def main():
    # 混合场景：多环境，四种类循环（可改为 8、16 等 4 的倍数）
    num_envs = 4
    scene_list = [SCENE_EMPTY, SCENE_CYLINDER, SCENE_DOOR, SCENE_BOX]
    scene_types = [scene_list[i % 4] for i in range(num_envs)]
    test_scene_type = "mixed"

    print("\n=== Mixed scene layout ===")
    _counts = {}
    for _st in scene_types:
        _counts[_st] = _counts.get(_st, 0) + 1
    for _st, _n in sorted(_counts.items(), key=lambda x: str(x[0])):
        print(f"  {_st}: {_n} env(s)")
    ...
    ppo_update_interval_steps = 1024      # old: update_freq
    ckpt_save_every_update_epochs = 2    # old: save_every
    ppo_total_update_epochs = 20         # old: total_epochs
    ...
    while simulation_app.is_running():
        ...
        if not sft_collection_active:
            ...
            if step_count % ppo_update_interval_steps == 0 and len(rl_agent.buffer["obs"]) >= rl_agent.batch_size:
                print('reward_buffer len', len(rl_agent.buffer["obs"]))
