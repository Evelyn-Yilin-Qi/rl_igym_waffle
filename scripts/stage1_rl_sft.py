"""
Stage1 RL + Optional SFT

Design constraints:
- Do NOT modify scripts/stage1_rl.py.
- Keep stage1 scene loading logic unchanged: 16 envs, SCENE_EMPTY only.
...
def main():
    ...
    # SFT settings
    enable_sft_warmup = True          # old: ENABLE_SFT 表达是否开启SFT warmup阶段
    sft_collect_total_steps = 2000    # old: SUPERVISED_STEPS 表达SFT warmup阶段的总收集数据步数
    sft_opt_epochs = 50               # old: SUPERVISED_EPOCHS 表达SFT 采样之后的训练总epoch数

    # RL settings
    # - ppo_update_interval_steps: every N sim-steps trigger one PPO update check (old: update_freq)
    # - ppo_total_update_epochs: total PPO update epochs (max updates) (old: total_epochs)
    # - ckpt_save_every_update_epochs: save checkpoint every N PPO update epochs (old: save_every)
    ppo_update_interval_steps = 124#2048 # 表达PPO更新间隔的步数
    ppo_total_update_epochs = 30 # 表达PPO更新的总epoch数
    ckpt_save_every_update_epochs = 6 # 表达每个PPO更新epoch后保存checkpoint的间隔
    # Progress heartbeat is based on update interval.
    rl_progress_print_every_steps = max(1, ppo_update_interval_steps // 20)  # old: rl_progress_print_every

    # Output/log settings
    checkpoint_dir = os.path.join(PROJECT_ROOT, "checkpoints", "ppo_stage1_rl_sft")
    run_dir = os.path.join(PROJECT_ROOT, "runs", "ppo_stage1_rl_sft")
    ...
    # Env setup (unchanged stage1 topology)
    ...
    # Network/Algo/Reward (keep modular style from stage1_rl.py)
    ...
    print("\n=== Start Stage1 RL+SFT ===")
    print(
        f"num_envs={num_envs}, scene=EMPTY only, use_supervised={enable_sft_warmup}, "
    ...
    while simulation_app.is_running():
        ...
        if sft_collection_active and step_count < sft_collect_total_steps:
            # Multi-env SFT collection: each env uses its own obs/lidar/intent, then batch-train later.
            for i in range(num_envs):
            ...
            for i in range(num_envs):
                if obstacle_collision[i]:
                    ...
                    # Align with V3: sample each step
                    if step_count % 1 == 0:
