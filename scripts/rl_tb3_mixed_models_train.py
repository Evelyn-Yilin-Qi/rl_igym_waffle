"""
混合场景 ``rl_tb3_mixed.py``：依次用 5 个预训练 ``best_actor_*.pth`` 各跑一遍（每轮新进程）。

**不要用 ``isaac_python`` 运行本编排脚本**（见 ``rl_tb3_subprocess_runner``）。请用::

    python3 scripts/rl_tb3_mixed_models_train.py
"""
...
def main() -> None:
    ...
    script = os.path.join(PROJECT_ROOT, "scripts", "rl_tb3_mixed.py")
    for model in MODELS:
